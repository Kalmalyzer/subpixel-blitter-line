...
.rightWard_yMajor
		move.w	#SubPixelBlitterLine_Mask,d4
		move.w	#SubPixelBlitterLine_Mask,d5
		sub.w	d0,d4
		sub.w	d1,d5
		and.w	#SubPixelBlitterLine_Mask,d4			; prestep_x = SubPixelBlitterLine_Mask - (x0 & SubPixelBlitterLine_Mask)
		and.w	#SubPixelBlitterLine_Mask,d5			; prestep_y = SubPixelBlitterLine_Mask - (y0 & SubPixelBlitterLine_Mask)

		asr.w	#SubPixelBlitterLine_Bits,d0			; start_x = x0 >> SubPixelBlitterLine_Bits
		asr.w	#SubPixelBlitterLine_Bits,d1			; start_y = y0 >> SubPixelBlitterLine_Bits
		subq.w	#1,d1
		...
		lsl.w	#SubPixelBlitterLine_Bits,d2
		lsl.w	#SubPixelBlitterLine_Bits,d3

		add.w	d5,d4			; bltapt = dx*prestep_y-dy*prestep_x

		move.w	d2,bltbmod(a6)	; bltbmod = dx<<SubPixelBlitterLine_Bits

		add.w	d2,d3
		move.w	d3,bltamod(a6)	; bltamod = (dx-dy)<<SubPixelBlitterLine_Bits

		move.w	d4,bltapt+2(a6)
		
		asr.w	#SubPixelBlitterLine_Bits,d7			; end_y = (y1 >> SubPixelBlitterLine_Bits)
...
.leftWard_yMajor
		move.w	d0,d4
		move.w	#SubPixelBlitterLine_Mask,d5
		and.w	#SubPixelBlitterLine_Mask,d4
		sub.w	d1,d5
		subq.w	#1,d4			; prestep_x = (x0 & SubPixelBlitterLine_Mask) - 1
		and.w	#SubPixelBlitterLine_Mask,d5			; prestep_y = SubPixelBlitterLine_Mask - (y0 & SubPixelBlitterLine_Mask)
...
.rightWard_xMajor
		...
		asr.w	#SubPixelBlitterLine_Bits,d0
		asr.w	#SubPixelBlitterLine_Bits,d1

		subq.w	#1,d0			; start_x = (x0 >> SubPixelBlitterLine_Bits) - 1
		subq.w	#1,d1			; start_y = (y0 >> SubPixelBlitterLine_Bits) - 1
		...
		move.w	d3,bltbmod(a6)		; bltbmod = dy<<SubPixelBlitterLine_Bits

		add.w	d2,d3
		move.w	d3,bltamod(a6)		; bltamod = (dy-dx)<<SubPixelBlitterLine_Bits
		...
		move.w	d6,d2
		move.w	d7,d3
		and.w	#SubPixelBlitterLine_Mask,d2
		and.w	#SubPixelBlitterLine_Mask,d3
		subq.w	#1,d2
		subq.w	#1,d3
		...
		move.w	d6,d7
		asr.w	#SubPixelBlitterLine_Bits,d7			; end_x = (x1 >> SubPixelBlitterLine_Bits)
...
.leftWard_xMajor
		...
		addq.w	#1,d0			; start_x = (x0 >> SubPixelBlitterLine_Bits) + 1
		subq.w	#1,d1			; start_y = (y0 >> SubPixelBlitterLine_Bits) - 1
		...
		move.w	#SubPixelBlitterLine_Mask,d2
		sub.w	d6,d2
		move.w	d7,d3
		and.w	#SubPixelBlitterLine_Mask,d2
		and.w	#SubPixelBlitterLine_Mask,d3
